...
class BookingRepository extends BaseRepository
{
     ...
     * @return array
     */
    public function getUsersJobs($user): array
    {
        $usertype = '';
        $emergencyJobs = array();
        $noramlJobs = array();
        ...
        if ($jobs) {
            ...
            $noramlJobs = collect($noramlJobs)->each(function ($item, $key) use ($user) {
                $item['usercheck'] = Job::checkParticularJob($user, $item);
            })->sortBy('due')->all();
        }
    ...
    public function store($user, $data): array
    {
        ...
        $response = [];
        if ($user->user_type == config('app.customerole_id')) {
            $cuser = $user;
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
     ...
     * @return void
     */
    public function jobEnd($post_data = array()): void
    {
        ...
        Event::dispatch(new SessionEnded($job, ($post_data['userid'] == $job->user_id) ? $tr->user_id : $job->user_id));
    ...
    public function sendSMSNotificationToTranslator($job): int
    {
        ...
        foreach ($translators as $translator) {
            ...
            SendSMSHelper::send(config('app.sms_number'), $translator->mobile, $message);
        }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
     ...
     * @return mixed
     */
    public function updateJob($job, $data, $cuser): mixed
    {
     ...
     * @return mixed
     */
    private function changeStatus($job, $data, $changedTranslator): mixed
    {
    ...
    {
        // Its also moved to job and used queue
        $user = $job->user()->first();
        if (!empty($job->user_email)) {
            $email = $job->user_email;
        } else {
    ...
    public function getPotentialJobs($cuser): array
    {
    ...
    public function endJob($post_data): array
    {
        ...
        // Must be moved into service and job
        $mailer = new AppMailer();
        $mailer->send($email, $name, $subject, 'emails.session-ended', $data);
        ...
        Event::dispatch(new SessionEnded($job, ($post_data['user_id'] == $job->user_id) ? $tr->user_id : $job->user_id));
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == config('app.super_admin_role_id')) {
            $allJobs = Job::query();
